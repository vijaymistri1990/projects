...
import { error_log_message, date_format, replace_str_arr, str_replace, mysql_real_escape_string, isRealValue } from "common-utils";
import { conn } from "./db_connect.js";
import config from "../config/config.js";

const { TABLE_CLIENT_STORES, ACTION_DECRYPT, ALLOWED_IMG_TYPE } = config;
import { createLogDb } from "../helper/log_function.js";
...
const deleteRetNo = (table, where) => {
  try {
    ...
    if (!where) {
      const error_log = error_log_message(
        "0",
        "0",
        table,
        where,
...
function getWhereCondition(where_condition) {
  try {
    if (typeof where_condition == "undefined" || where_condition == "") {
      ...
      Array.isArray(where_condition) === true ||
      typeof where_condition === "object"
    ) {
      let where = [];
...
function getEncryptDecryptData(action, string) {
  try {
    ...
    let password_iv = createHash("sha256")
      .update(secret_iv)
      .digest("hex");
    let password_hash = createHash("sha256")
...
function hex2bin(hex) {
  ...
  for (; i < len; i += 2) bytes.push(parseInt(hex.substr(i, 2), 16));
      ...
      connection.query(query, value, function (errs, query_resource_obj) {
        if (errs) {
          ...
          const str =
            ...
            errs.stack;
          let log_arrs = {
            log: replace_str_arr(["'", "\\\\'"], ["'", "'"], str),
          };
          createLogDb("error_query_fail_query_transaction_function", log_arrs);
          resolve(false);
        } else {
          resolve(query_resource_obj.rows); // PostgreSQL returns rows in .rows property
        }
      });
    } catch (error) {
      let error_log = error_log_message(
        "0",
        "0",
        error,
        query,
...
function insertOnDuplicateUpdate(table, fields, update_fields) {
  try {
    ...
    let insert_query =
      ...
      values.join("','") +
      "')";
    let insert_on_conflict_query = `${insert_query} ON CONFLICT DO UPDATE SET ${update_fields};`;
    insert_on_conflict_query = str_replace(
      ["'NULL'", "'null'"],
      "NULL",
      insert_on_conflict_query
    );
    return query(insert_on_conflict_query);
  } catch (error) {
    let error_log = error_log_message(
      "0",
      "0",
...
const getStoreData = (shop = "") => {
  try {
    const where = {
      status: "1",
      store_name: mysql_real_escape_string(shop),
    };
    const common_field =
      "token,money_format,iana_timezone,store_name,store_client_id,currency,shop_name,onboarding_status,app_status,store_theme_id";
    return selectedRows(TABLE_CLIENT_STORES, common_field, where);
  } catch (error) {
    const error_log = error_log_message(
      0,
      "0",
...
const prepareData = async (reqData, reqQuery = {}) => {
  ...
  try {
    ...
    let store_client_id =
      reqQuery.store_client_id || reqData.store_client_id || "";
    if (!isRealValue(reqData.shopInfo)) {
      ...
      shop_info.token = shop_info.token
        ? getEncryptDecryptData(ACTION_DECRYPT, shop_info.token)
        : "";
      shop_info.store_client_id = shop_info.store_client_id
        ? getEncryptDecryptData(
            ACTION_DECRYPT,
            shop_info.store_client_id
          )
        : "";
      store_client_id = shop_info.store_client_id;
      shop = shop_info.store_name;
...
const multipleUpdates = (table, field, array, where, key, value) => {
  try {
    ...
    let sql = `UPDATE ${table} SET ${field}= CASE ${key}`;
    let where_arr = [];
    if (array_len > 0) {
      while (array_len--) {
...
const getFieldName = (tableName, exclude_fields = []) => {
  try {
    // PostgreSQL uses information_schema instead of SHOW COLUMNS
    let exclude_str =
      exclude_fields.length > 0
        ? `AND column_name NOT IN (${exclude_fields
            ...
            .join(",")})`
        : "";
    return `SELECT column_name FROM information_schema.columns WHERE table_name = '${tableName}' AND column_name NOT IN (SELECT column_name FROM information_schema.key_column_usage WHERE table_name = '${tableName}') ${exclude_str}`;
  } catch (error) {
    createLogDb("error_common_else_cht_file", {
      store_client_id: 0,
      type: "0",
...
async function storeImage(matches, res, pathImg) {
  ...
  return new Promise((resolve) => {
    try {
      ...
      if (
        matches.length != 3 ||
        !ALLOWED_IMG_TYPE.includes(matches[1]) ||
        file_size > 2
      ) {
        /* Do nothing */
        resolve(false);
        ...
        let writeStream = createWriteStream(target_path);
